...
def register_task_definition(ecs_client, service_name, image, environment, port_mappings :list, private_repo_creds, execution_role, task_role):
        ...
        containerDefinitions=[
            {
                ...
                'portMappings': [{**port_mappings[0], 'name': service_name}] if port_mappings else [{'name': service_name}]
            }
        ],
        ephemeralStorage={
        'sizeInGiB': 21
...
def create_service(ecs_client, cluster_name, service_name, task_definition, subnets, security_groups, port_mappings):
...
services = [
    ...
    #     },
    #     'portMappings': [{'containerPort': 8080, 'hostPort': 8080, 'protocol': 'tcp', 'appProtocol': 'http'}]
    # },
    # {
    #     'name': 'auth-api',
    ...
    #         #'ZIPKIN_URL': 'http://zipkin:9411/api/v2/spans'
    #     },
    #     'portMappings': [{'containerPort': 8081, 'hostPort': 8081, 'protocol': 'tcp', 'appProtocol': 'http'}]
    # },
    # {
    #     'name': 'todos-api',
    ...
    #         'ZIPKIN_URL': 'http://zipkin:9411/api/v2/spans'
    #     },
    #     'portMappings': [{'containerPort': 8082, 'hostPort': 8082, 'protocol': 'tcp', 'appProtocol': 'http'}]
    # },
    # {
    #     'name': 'users-api',
    ...
    #     },
    #     'portMappings': [{'containerPort': 8083, 'hostPort': 8083, 'protocol': 'tcp', 'appProtocol': 'http'}]
    # },
    {
        ...
        'portMappings': [{'containerPort': 6329, 'hostPort': 6329, 'protocol': 'tcp', 'appProtocol': 'http'}]
    },
    # {
    #     'name': 'zipkin',
    #     'image': 'openzipkin/zipkin',
    #     'environment': {},
    #     'portMappings': [{'containerPort': 9411, 'hostPort': 9411, 'protocol': 'tcp', 'appProtocol': 'http'}]
    # },
    # {
    #     'name': 'redis-queue',
    #     'image': 'redis',
    #     'environment': {},
    #     'portMappings': [{'containerPort': 6379, 'hostPort': 6379, 'protocol': 'tcp', 'appProtocol': 'http'}]
    # }
]
...
for service in services:
    ...
    create_service(
        ...
        port_mappings=service['portMappings'] if 'portMappings' in service else []
    )
